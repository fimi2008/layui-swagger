...
layui.use(['layer', 'element', 'form', 'nlaytpl', 'nswagger', 'ncmntool', 'upload'], function () {
    ...
    $("#req-btn").click(function () {
        var iptApiUrl = $("#iptApiUrl").val();
        contextPath = getContextPath(iptApiUrl);
        getRequestTree(iptApiUrl)
    });

    function init() {
        contextPath = getContextPath(window.location.pathname);
        var iptApiUrl="/v2/api-docs";
        if (contextPath){
            iptApiUrl = contextPath + iptApiUrl
        }
        $("#iptApiUrl").val(iptApiUrl);
        getRequestTree(iptApiUrl);
    }

    function getRequestTree(iptApiUrl){
        // 弹出加载框
